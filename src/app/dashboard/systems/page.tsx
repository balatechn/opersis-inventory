...
export default function SystemsPage() {
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...filtered.map((a) => [a.assetTag, a.productName, a.serialNumber || "", getCompanyLabel(a.company), a.department || "", a.location || "", a.status, String(a.cost || 0)]),
    ].map((r) => r.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url; link.download = "assets_report.csv"; link.click();
  ...
  const openEdit = (a: Asset) => {
    setEditItem(a);
    setForm({
      productName: a.productName, serialNumber: a.serialNumber || "", make: a.make || "",
      config: a.config || "", osVersion: a.osVersion || "", company: a.company,
      department: a.department || "", location: a.location || "", vendorDetails: a.vendorDetails || "",
      cost: a.cost ? String(a.cost) : "", status: a.status, invoiceNumber: a.invoiceNumber || "",
      warrantyPeriod: a.warrantyPeriod || "", remarks: a.remarks || "",
    });
    setEditOpen(true);
  };
  ...
  const handleUpdate = async () => {
    if (!editItem) return;
    setSaving(true);
    await fetch(`/api/systems/${editItem.id}`, {
      method: "PUT",
          ...
          <CardContent className="p-4">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search by name, tag, or serial..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
              </div>
        ...
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-4 py-2">
              ...
              <div className="grid gap-2"><Label>Configuration</Label><Input value={form.config} onChange={(e) => setForm({ ...form, config: e.target.value })} /></div>
